...
def kb_main_menu():
    ...
    b.row(types.InlineKeyboardButton(text="💬 Текстовый формат",     callback_data="info_text"))
    b.row(types.InlineKeyboardButton(text="🎙 Аудио формат",         callback_data="info_audio"))
    b.row(types.InlineKeyboardButton(text="🎥 Видео формат",         callback_data="info_video"))
    b.row(types.InlineKeyboardButton(text="🔒 Закрытая группа",      callback_data="info_group"))
    return b.as_markup()
...
def kb_after_diagnosis():
    b = InlineKeyboardBuilder()
    b.row(types.InlineKeyboardButton(text="💬 Продолжить в тексте", callback_data="info_text"))
    b.row(types.InlineKeyboardButton(text="🎙 Продолжить в аудио",  callback_data="info_audio"))
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "Привет. Я Евгений Чуклов ☀️\n\n"
        "Каждый день ко мне приходят люди с разными запросами — "
        "но корень всегда один: непонимание того, *Кто Ты Есть*.\n\n"
        "Начни с бесплатной *Диагностики Сознания* — три вопроса, "
        "и ты увидишь себя иначе.\n\n"
        "Обнял сердцем 🫶",
...
async def cb_info_text(callback: types.CallbackQuery):
    await callback.message.edit_text(
        "💬 *Текстовый формат*\n\n"
        "Живой диалог со мной — в любое время, в своём темпе.\n"
        "Я отвечаю глубоко, точно, из потока.\n\n"
        "• До 50 сообщений в месяц\n"
        "• Персональные ответы в моём голосе\n"
        "• Стоимость: [будет указана]\n\n"
        "_Оплата и подключение — в следующем обновлении_",
...
async def cb_info_audio(callback: types.CallbackQuery):
    await callback.message.edit_text(
        "🎙 *Аудио формат*\n\n"
        "Голосовые ответы — мой голос, моя интонация, моя энергия.\n"
        "Слушаешь как живое послание.\n\n"
        "• До 20 голосовых в месяц\n"
        "• Клонированный голос Евгения\n"
        "• Стоимость: [будет указана]\n\n"
        "_Этот формат будет доступен в следующем обновлении_ 🔜",
...
async def cb_info_video(callback: types.CallbackQuery):
    await callback.message.edit_text(
        "🎥 *Видео формат*\n\n"
        "Видео-послание с моим аватаром — смотришь и чувствуешь живое присутствие.\n\n"
        "• До 10 видео в месяц\n"
        "• Аватар Евгения Чуклова\n"
        "• Стоимость: [будет указана]\n\n"
        "_Этот формат будет доступен позже_ 🔜",
        reply_markup=kb_back(),
        parse_mode="Markdown"
    )
    await callback.answer()
...
async def cb_info_group(callback: types.CallbackQuery):
    await callback.message.edit_text(
        "🔒 *Закрытая группа*\n\n"
        "Пространство для тех, кто идёт по пути пробуждения Сознания.\n\n"
...
async def cb_pay_text(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "✅ Доступ открыт!\n\n"
        "Теперь ты можешь писать мне напрямую — отвечу из потока.\n"
        "Напиши свой первый вопрос или запрос 👇\n\n"
...
async def cb_pay_group(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        f"✅ Доступ открыт!\n\n"
        f"🔒 Ссылка на закрытую группу:\n{GROUP_LINK}\n\n"
        f"До встречи внутри. Обнял сердцем 🫶"
    )
    await callback.answer()
...
async def handle_chat(message: types.Message, state: FSMContext):
    ...
    if ud["msg_count"] >= MAX_MESSAGES:
        await message.answer(
            "☀️ Ты исчерпал лимит сообщений на этот месяц.\n\n"
            "Для продолжения — обнови подписку.",
            reply_markup=kb_main_menu()
        )
        await state.clear()
        return
